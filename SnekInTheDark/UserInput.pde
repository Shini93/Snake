 ...
 ********************/
void mousePressed() {
  //log.append("UserInput.mousePressed");
...
void mouseReleased() {
  //log.append("UserInput.mouseReleased");
  if(GameStart == false)
    return;
  changeSnakeSpeed(-snake[0].upgrades.SpeedValue);
}
...
void keyPressed() {
  //log.append("UserInput.keyPressed");
...
void keyReleased() {
  //log.append("UserInput.keyReleased");
  directionKey = 0;
...
void changeSnakeSpeed(float value) {
  //log.append("UserInput.changeSnakeSpeed");
...
void applyMissile() {
  //log.append("UserInput.applyMissile");
...
void backPressed() {
  //log.append("UserInput.backPressed");
  fill(#222222);
  ...
  music.stop();
  if(music.isPlaying())
    music = new SoundFile(this, "Light Ambient 3 (Loop).wav");
}
