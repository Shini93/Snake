...
class Level {
  ...
  void callBlocks(byte level,int[][] pos) {
    //log.append("level.callBlocks");
    snake[0].body.get(0).pos = new PVector(pos[0][0],pos[0][1]);
    winsize = round( 0.3 * worldSize.x) + 5 * ceil( level * 2.0) - 50;    //winsize higher every 5 level, or level size incease
