class UpgradeHandler {
  ...
  Boolean speed = false;
  int glow = 30;       //glow radius
  int glowdistance = 50;    //increments
  int magnetDist = 6;
  int magnetPullDist = 20;
  int magnetPullStr = 100;
  Boolean[] PreUpgradeEaten = new Boolean[10];
  int[] PreUpgradePos = new int[10];
  float RayRadius = PI/16;
  float[] chanceValue = {1, 2, 1, 5, 5, 3, 3, 4};  //missile, speed, minisnake, raydist, raywidth, glow, magnet, lantern
  int RayDistance = 200;
  int MaxSpecials = 0;
  int SpeedValue = 1;
  ...
  void addUpgrade(int count) {
    ...
    if (specials.get(count).Specialvalue ==0) {
      ...
      RayDistance+=30-30*(RayDistance/400);
      if (RayDistance > 480)
        RayDistance = 480;
    } else if (specials.get(count).Specialvalue == 4)
      RayRadius += (0.005*PI) * chanceValue[4] ;
    else if (specials.get(count).Specialvalue == 5) {
      glow += glowdistance;
      glowdistance = round(max(glowdistance * 1.2, glowdistance + 30));
      //Set glow to Snake
      //expanse glow with each upgrade
      //glow lights up parts where snake is
    } else if (specials.get(count).Specialvalue == 6)
      magnetDist += 6;
     else if (specials.get(count).Specialvalue == 7){
      laternen.add( new Laterne(snake[snakeID].body.get(0).pos, laternen.size()));
     }
  ...
  float boughtUpgradeMultiplicator(int upgrade) {
    //log.append("UpgradeHandler.boughtUpgradeMultiplicator");
    switch(upgrade) {
      //Speed
    case 0:
  ...
  void PreUpdate() {
    //log.append("UpgradeHandler.PreUpdate");
    for (int i = 0; i<10; i++) {
  ...
  int adjustSpecialValue() {
    //log.append("UpgradeHandler.adjustSpecialValue");
    int allVal = 0;
