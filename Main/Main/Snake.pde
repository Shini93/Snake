...
class Snake extends PrimalSnake {
  ...
  float findFood(ArrayList <Integer> gridIDs, int g, int f, int foodID) {
    ...
    if (distance <= upgrades.glowdistance) {
      food.get(foodID).countSnake = SLength;
    }
    if (distance >= ((food.get(foodID).size+size)/2) + upgrades.magnetDist )
      return distance;
  ...
  void testSnake() {
    ...
    for (Snake s : snake) {
      ...
      for (int b = 0; b<s.SLength; b++) {
        float distance = dist(s.body.get(b).pos.x, s.body.get(b).pos.y, newpos[0], newpos[1]);
